  ...
  render() {
    ...
    return (
      <View style={styles.container}>
        ...
        <Text style={globalStyles.topLabel}>
          {plantData.name != null
            ? ("How to grow " + plantData.name + ": ").toUpperCase()
            : ""}
        </Text>
        <ScrollView showsVerticalScrollIndicator={false}>
          {guideData.chapterTitles.length > 0 ? (
            guideData.chapterTitles.map(title => {
              const added = false;
              return (
                <View style={styles.row} key={title}>
                  <Touchable
                    onPress={() => {
                      this.props.navigation.navigate("GuidesSteps", {
                        data: guideData,
                        chapter:
                          guideData.chapterTitles.findIndex(x => x == title) + 1
                      });
                    }}
                    onLongPress={null}
            ...
            <Text style={styles.noresults}>
              Unfortunately, there are no guide chapters registered.
            </Text>
          )}
        </ScrollView>
      </View>
