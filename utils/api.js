...
export const getMyPlants = () => {
  ...
  for(let i in myPlants){
    plants[i] = []; // multidimensional array,issue solution: https://stackoverflow.com/a/42666851
    for(let y = 0; y < myPlants[i].length; y++){ 
      let plant = myPlants[i][y];
      if(plant.steps.length > 0 && Object.keys(plant.steps).length > 0 && plant.steps[0].title != null && plant.steps[0].desc != null){ // check if steps not empty
          plants[i][y] = plant; 
      }
    }
  } 
...
export const getAllPlants = () => {
  ...
  for(let i in allPlants){
    plants[i] = []; // multidimensional array,issue solution: https://stackoverflow.com/a/42666851
    for(let y = 0; y < allPlants[i].length; y++){ 
      let plant = allPlants[i][y];
      if(plant.steps.length > 0 && Object.keys(plant.steps).length > 0 && plant.steps[0].title != null && plant.steps[0].desc != null){ // check if steps not empty
          plants[i][y] = plant; 
      }
    }
